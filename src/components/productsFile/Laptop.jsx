...
const Laptop = () => {
    ...
    useEffect(() => {
        let LAP = products.filter((x) => x.category == 'laptops')
        setLaptops(LAP)
    }, [])
    return (
        <>
            <div className='relative'>
                <div className=" bg-slate-100 h-32 mt-5">
                    <li className='absolute list-none left-32 top-16 text-gray-600 text-sm'><NavLink to={'/'}>Home</NavLink>/<NavLink to={-1}>All-Products</NavLink> / <span className='text-orange-600'>Laptops Collection</span></li>
                </div>
            </div>
            <div>
                <h3 className='text-center mt-7 text-4xl text-orange-600'>All Laptops</h3>
            </div>
