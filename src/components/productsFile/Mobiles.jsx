...
const Mobiles = () => {
    ...
    useEffect(() => {
        let MOBS = products.filter((x) => x.category == 'mobiles')
        setMobiles(MOBS)
    },)
    return (
        <>
            <div className='relative'>
                <div className=" bg-slate-100 h-32 mt-5">
                    <li className='absolute list-none left-32 top-16 text-gray-600 text-sm'><NavLink to={'/'}>Home</NavLink>/<NavLink to={-1}>All-Products</NavLink> / <span className='text-orange-600'>Mobiles Collection</span></li>
                </div>
            </div>
            <div>
                <h3 className='text-center mt-7 text-4xl text-orange-600'>All Mobiles</h3>
            </div>
